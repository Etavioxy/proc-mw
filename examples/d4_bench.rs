//! D4 性能 · 局部加法 + 空链透明 —— 基准（Debug/Release 双构建）
//!
//! 复现：
//!   cargo run --example d4_bench --release   # Release
//!   cargo run --example d4_bench             # Debug（空链透明在非 Release 下也成立）
...
// 输入逐迭代变化防常量折叠；fn 指针 black_box 防去虚拟化
...
fn main() {
    ...
    println!(
        "[T1] 空链透明：裸调用 {:.3} ns vs 空链 {:.3} ns → 差值 {:.3} ns {}",
        bare,
        empty_chain,
        diff,
        if diff.abs() < 3.0 { "（无异）✓" } else { "（可测差异）" }
    );

    // ---- T2：局部加法（链长 vs 边际成本）----
    println!("[T2] 链长 vs 每迭代成本");
    ...
    println!(
        "[T3] 4×Add {:.3} ns vs 4×FnPtr {:.3} ns → 每间接调用 ≈ {:.3} ns",
        t_inline,
        t_indirect,
        (t_indirect - t_inline) / 4.0
    );
    ...
    chains[25].push(Node::FnPtr(mul3));
    let after = total(&chains);
    let meas_delta = after - before;
    println!(
        "[T4] 50 链改 1 条：全局每迭代 {:.3} → {:.3} ns，增量 {:.3} ns（局部，其余 49 条不受影响）",
        before, after, meas_delta
    );
    println!("---");
    println!("判读：空链透明（含 Debug）+ 局部加法。详细结论见 docs/results/d4_perf.md");
}
