...
fn compile_rustc_driver(source: &str, out_dir: &std::path::Path) -> std::path::PathBuf {
...
fn main() {
    ...
    // rustc_driver 管线（每次真实编译）
    let t = Instant::now();
    for i in 0..iters {
        compile_rustc_driver(mw_source(), &out_dir);
    }
    let rd_total = t.elapsed().as_secs_f64();
    println!("rustc_driver : {iters} 次 {rd_total:.2}s，平均 {:.2}s/次（进程内，无子进程）", rd_total / iters as f64);

    // 产物大小
    let cargo_so = build_plugin_cached("cmp_cargo", mw_source(), &out_dir).unwrap();
    let rd_so = compile_rustc_driver(mw_source(), &out_dir);
    let c_size = fs::metadata(&cargo_so).map(|m| m.len()).unwrap_or(0);
    let r_size = fs::metadata(&rd_so).map(|m| m.len()).unwrap_or(0);
    println!("产物大小    : cargo={}B, rustc_driver={}B", c_size, r_size);
}
