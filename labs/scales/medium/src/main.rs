...
"#;
    ...
    let so = build_plugin_cached("med_audit", src, &std::env::temp_dir()).expect("运行期编译");
    ...
    println!("[中型] 运行期编译中间件: {compile_ms}ms");
    println!("[中型] {} handler 经链执行: {exec_ms}ms，{:.2} M ops/s，总结果 {}", n_handlers, n_handlers as f64 / (exec_ms as f64 / 1000.0) / 1e6, total);
    println!("[中型] metrics: calls={} successes={} errors={} ✓", metrics.calls(), metrics.successes(), metrics.errors());
    assert_eq!(metrics.calls(), n_handlers);
    assert_eq!(metrics.errors(), 0);
    println!("[中型] 中型系统（~万 LOC）proc-mw 方法验证通过 ✓");
}
