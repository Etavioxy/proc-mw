...
use proc_mw::dispatch::{Ctx, Mw, MwError, Node};
...
fn rate_limit_rejects_overflow() {
    ...
    let chain = Chain::new(vec![Node::Dyn(Box::new(limiter))]);
...
fn rate_limit_shared_state_clone() {
    ...
    let chain2 = Chain::new(vec![Node::Dyn(limiter.box_clone())]); // 先 box_clone（借用）
    let chain1 = Chain::new(vec![Node::Dyn(Box::new(limiter))]); // 再 move
